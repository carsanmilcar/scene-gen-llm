...
- `focus`: leading category: `wash`, `puntuales`, `special`, `mixed`.
...
- `puntuales_fade_color`: slow transitions on accents over static wash.
- `puntuales_sync_beats`: on selected beats (e.g., 1 and 3).
...
function select_scene(context):
    ...
    return choose_weighted_by_energy(candidatas)
```
