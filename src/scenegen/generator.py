...
def generate_scenes_for_song(
    ...
    """Generate a ``SceneSet`` using rule-based selection or the LLM fallback."""
    ...
    # Rule-based path for multiple contexts (batch SceneSet).
    if scene_contexts:
...
def _generate_from_catalog(
    ...
    if not catalog:
        logger.warning("Scene catalog is empty; falling back to LLM")
        return None

    selected = select_scene(context, catalog)
    if not selected:
        logger.warning("No scene selected; falling back to LLM")
        return None
    ...
    logger.info("Rule-based scene selected: %s", selected.name)
...
def _generate_multiple_from_catalog(
    ...
    for ctx in contexts:
        # Propagate last_* if the context does not provide them to avoid repetition.
    ...
    if not scenes:
        logger.warning("No scenes generated for the provided contexts")
        return None

    title = f"Generated for {rig.name} (rule-based batch)"
    logger.info("Rule-based batch scenes generated: %d", len(scenes))
    return SceneSet(title=title, scenes=scenes)
