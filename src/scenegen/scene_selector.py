"""Rule-based semantic scene selector inspired by the SynkroDMX engine."""
...
@dataclass
class SceneContext:
    """Music context and recent state used to pick the next scene."""
...
@dataclass
class SemanticScene:
    """Semantic (non-DMX) scene with the model parameters."""
    ...
    def from_dict(cls, data: dict) -> "SemanticScene":
        """Build a ``SemanticScene`` from a plain dictionary."""
        ...
        return cls(
            ...
            meta={
                ...
                if k not in {"name", "energy", "palette", "motion", "strobe", "focus"}
            },
        )
...
def select_scene(context: SceneContext, catalog: Sequence[SemanticScene]) -> Optional[SemanticScene]:
    """Select a scene applying the methodology filters."""

    initial = list(catalog)
    logger.debug("Starting selection with %d scenes in catalog", len(initial))

    candidates = _filter_by_energy(initial, context.energy, delta=1)
    logger.debug("After energy filter (+/-1) there are %d candidates", len(candidates))

    if context.last_palette:
        before = len(candidates)
        candidates = [s for s in candidates if s.palette != context.last_palette]
        logger.debug(
            "Palette filter (different from %s): %d -> %d", context.last_palette, before, len(candidates)
        )
    ...
    if context.energy < 3:
        ...
        logger.debug("Focus wash filter (energy<3): %d -> %d", before, len(candidates))
    ...
    if not context.strobe_allowed:
        ...
        logger.debug("Strobe-disabled filter: %d -> %d", before, len(candidates))

    if not candidates:
        logger.warning("No candidates after filters; relaxing energy criteria")
        candidates = initial
...
def _weighted_choice_by_energy(candidates: Sequence[SemanticScene], target_energy: int) -> Optional[SemanticScene]:
    """Pick a scene weighting closeness to the target energy."""
    ...
    for scene in candidates:
        ...
        weights.append(max(1, 3 - diff))  # exact energy weighs more

    choice = random.choices(candidates, weights=weights, k=1)[0]
    logger.debug(
        "Selected scene: %s (energy=%d) with %d candidates",
        choice.name,
        choice.energy,
        len(candidates),
    )
